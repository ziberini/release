...
def main():
    ...
    if not token:
        raise ValueError(
            "PERSONAL_ACCESS_TOKEN environment variable is not set")
    ...
    for repo_info in config['repositories']:
        if repo_info['enabled']:
            ...
            # Join release notes with new line character
            release_notes_str = "\n".join(release_notes)
            try:
                repo = g.get_repo(repo_name)
                if not tag_exists(repo, tag):
                    print(
                        f"Tag {tag} not found in {repo_name}, updating deployment.yaml")
                    payload = json.dumps({
                        "ref": "xyz",
                        "inputs": {
                            "tag": tag,
                    ...
                    subprocess.run(['git', 'config', '--global',
                                   'credential.helper', 'store'], check=True)
                    ...
                    print(
                        f"Tag {tag} already exists in {repo_name}, skipping update.")
            except Exception as e:
