    ...
    for repo_info in config['repositories']:
        if repo_info['enabled']:
            ...
            try:
                ...
                subprocess.run(['git', 'add', deployment_path, 'release_notes.txt'], check=True)
                subprocess.run(['git', 'commit', '-m', f'Update deployment.yaml with tag {tag} and add release_notes.txt'], check=True)
